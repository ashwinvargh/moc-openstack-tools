...
moc_standards = {'subnet':10, 'router':10, 'port':10, 'network':5, 'floatingip':2, 
'security_group':-1,'security_group_rule':-1, 'ram': 51200, 'gigabytes':1000, 
'snapshots':10, 'volumes':10, 'injected_file_content_bytes':10240, 'injected_files':5,
'metadata_items':128, 'instances':10,'cores':20} 


def matches_moc_quotas(project_quotas):
  quotas_match = True
  for each_quota in moc_standards:
    if moc_standards[each_quota] == project_quotas[each_quota]:
      quotas_match = quotas_match*True
    else:
      quotas_match = quotas_match*False
  return quotas_match
...
for qset in all_neutron_quotas:  
  ...
  try:
    ...
    if not matches_moc_quotas(combined_quotas):
      print project
